...
const STORAGE_KEY = 'nim_V2NIMStorageService_shortUrlToLong_params';
...
const ShortUrlToLongPage = () => {
  ...
  const onFinish = async (values: FormValues) => {
    ...
    const [error, apiResult] = await to(() =>
      window.nim?.V2NIMStorageService.shortUrlToLong(values.url.trim())
    );
  ...
  const handleOutput = () => {
    ...
    const statement = `const longUrl = await window.nim.V2NIMStorageService.shortUrlToLong("${values.url.trim()}");`;

...
const STORAGE_KEY = 'nim_V2NIMStorageService_uploadFileWithMetaInfo_params';
...
const UploadFileWithMetaInfoPage = () => {
  ...
  const uploadFileWithMetaInfo = async (task: any, metaInfo?: any) => {
    ...
    // 调用上传文件API (带元信息)
    const [error, result] = await to(() =>
      window.nim?.V2NIMStorageService.uploadFileWithMetaInfo(task, onProgress)
    );
  ...
  sceneName: "${values.sceneName}"
    ...
    const uploadStatement = `const result = await window.nim.V2NIMStorageService.uploadFileWithMetaInfo(uploadTask, (progress) => {

...
interface AddConversationsToGroupFormValues {
  groupId: string;
  conversationIds: string[];
}

const defaultAddConversationsToGroupFormValues: AddConversationsToGroupFormValues = {
  groupId: '',
  conversationIds: [],
};

// 持久化存储最终执行的参数
const storageKey = `V2NIMConversationGroupService.addConversationsToGroup`;

const AddConversationsToGroupPage = () => {
  // 获取初始值，优先从 localStorage 中获取
  const getInitialValues = (): AddConversationsToGroupFormValues => {
    try {
      const stored = localStorage.getItem(storageKey);
      if (stored) {
        const parsedStored = JSON.parse(stored);
        return { ...defaultAddConversationsToGroupFormValues, ...parsedStored };
      }
    } catch (error) {
      console.error('Failed to parse stored values:', error);
    }
    return defaultAddConversationsToGroupFormValues;
  };
  ...
  // 表单提交: 触发 API 调用
  const handleAddConversationsToGroup = async (values: AddConversationsToGroupFormValues) => {
    if (!(window.nim && window.nim.V2NIMLoginService.getLoginUser())) {
      message.error('NIM SDK 尚未初始化和登录');
      return;
    }
    ...
    // 打印 API 入参
    console.log('API V2NIMConversationGroupService.addConversationsToGroup execute, params:', {
      groupId,
      conversationIds,
    });
    ...
    const [error] = await to(() =>
      window.nim?.V2NIMConversationGroupService.addConversationsToGroup(groupId, conversationIds)
    );

    if (error) {
      message.error(`添加会话到分组失败: ${error.toString()}`);
      console.error('添加会话到分组失败:', error.toString());
    } else {
      message.success('添加会话到分组成功');
      console.log('添加会话到分组成功, groupId:', groupId, 'conversationIds:', conversationIds);
      // 操作成功后，重新获取会话分组列表
      getConversationGroupList();
    }
    // finally
  ...
  const handleReset = () => {
    ...
    // 重置表单为默认值
    form.setFieldsValue(defaultAddConversationsToGroupFormValues);
  ...
  const handleOutput = () => {
    ...
    if (!conversationIds || conversationIds.length === 0) {
      message.error('请先选择要添加的会话');
      return;
    }

    const callStatement = `await window.nim.V2NIMConversationGroupService.addConversationsToGroup("${groupId}", ${JSON.stringify(conversationIds)});`;

    console.log('V2NIMConversationGroupService.addConversationsToGroup 调用语句:');
  ...
  return (
    <div className={styles.formContainer}>
      <Form
        ...
        labelCol={{ span: 4 }}
        wrapperCol={{ span: 20 }}
        onFinish={handleAddConversationsToGroup}
        style={{ marginTop: 24 }}
      ...
      >
        ...
        <Form.Item
          ...
          tooltip="选择要添加到分组的会话"
          rules={[{ required: true, message: '请选择要添加的会话' }]}
        >
          <Select
            mode="multiple"
            placeholder="请选择要添加到分组的会话"
      ...
      <Card title="使用说明" style={{ marginTop: 16 }} size="small">
        <ul style={{ margin: 0, paddingLeft: 20 }}>
          <li>
            <strong>功能：</strong>将指定的会话添加到云端会话分组中
          </li>
          <li>
          ...
          <li>
            <strong>用途：</strong>将会话归类到指定分组，便于统一管理和展示
          </li>
        </ul>
      </Card>
      ...
      >
        <ul style={{ margin: 0, paddingLeft: 20, color: '#d46b08' }}>
          ...
          <li>同一会话可以属于多个分组</li>
          <li>分组和会话都必须选择，不能为空</li>
          <li>操作成功会触发相关事件，变更会同步到云端</li>
          <li>如果会话已在该分组中，重复添加不会报错</li>
        </ul>
      </Card>
    </div>
  );
...
export default AddConversationsToGroupPage;
